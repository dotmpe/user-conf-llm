---
title: Project conventions (+User-Conf-LLM edition)
status: draft
category: [ convention ]
audience: [ developer ]
tag:
...
Preamble:
: I am developing a generative shell-script system for command-substitution tooling (User-Scripts).
  I want rich (inline and contextually structured) documentation, and dense or (controlled) natural language [CNL] for readable strings, and explicit schemas for the generative parts, and (eventually too) a path toward using Lua for faster data and heuristics, and to further explore optimization through elimation of Bash, or by layering of Bash on Lua, or by deferring to compiled spin-off projects.
  I need help better testing, documenting flows, and to keep a low-key approach while developing solid tools and habits for writing source, making distributions, doing deployment, and managing environments for testing, diagnostics, etc. etc.
  Help me explore the design space, propose schemas, and produce both explanatory text and concrete artifacts.

## Getting started

- **Scope and platform of tools**: classic and novel \*nix style tools, with in-terminal, text-based UI.
  Debian (or Apt based) systems (my main installation go-to for 20+ years) have been available as open-source software [OSS] operating system distributions since 1993 (Linux kernel itself exists since 1991).

  This brings with it a philosophy in OS design with it, that is perhaps best captured in Plan 9 (that does name space binding), more fanatically than Linux has.
  We have file based organisation (and IPC, but no binding or true name spaces), host environments, and user access within the context of consoles, tty, Xorg/X11 server(s) and everything can run on that.

  TODO: can we mention/introduce the Debian lifecycle and channels just a bit, to get clear picture of platform as entity in time

- **Primary language**: We want to start by using the classic shell interpreter to its full extent, and not getting in the way of the existing system. 
  That means learning about and sometimes dealing with /bin/sh and other shells.

  * The body of work is already written in Bash, and we should target a modern (4.3+) version to avoid needing to carefully pick features prematurely (cf. pre-processing goals that can alleviate/mitigate compatibility concerns).
  ...
  * TODO: document Bash-specific setup/invocation/diagnostic/triage flows, may be scenarios and cases, may be not here--but to verify exhaustive and complete workflow. And that is in parallel to working out the exact name spaces or "shell contexts" there are (cf. remarks on contexts in 'Writing scripts' section).
  ...
  There is currently no reason to document other specific language requirements, other than Bash 4.3+.
  And no need to consider existing other shells.

- **Generative scripts and incremental builds** enable a system that potentially extends itself, and even rewrites.

  Without going into any details we can still say the user (or system itself) needs to establish control(s).

  XXX: even though the default/build-select is fairly generic and compatible with skeleton or template setups it is still all source/manual authored script

- **Reproducible setups**: Docker should help in testing scripts, and to run them in isolation on vanilla Bash setups.
  That allows close tabs on what is used in the dev/test runs and what is needed in deployment profile cq. bare-bone bootstrap scripts.

  TODO: make configure setup github CI, as well as local docker for local CI

* For per-project setup, the package.yaml is used as a sort of "catch-all" metadata storage. 
  It should be documented more formally (with schema), but for now roughly it is defined as a list-bag of different package.* and other JSON/TOML/YAML metadata blocks in one file--this is to keep down the clutter and try bring the work tree down to clean states.
...
- **Documentation** is written in Markdown.
  ...
  It is aside of the source also the secondary source for schema and data.

* Schema from LinkML YAML format documents (and used to validate YAML data, or to validate or handle data in downstream projects using code generated from schema).
...
* This document is one of several CONVENTIONS and AGENT files, found at ``doc/`` for several projects.
  ...
  The CONVENTIONS document shares several parallel copies.
  These keep versions and editions with changes that do not fit-in with the internal structure well; the copies help to keep easy to compare parallel versions, sharing certain stanza's and overall structure while allowing to focus convention editions on a particular project and/or restricted to a specific stage and task.

- Versions are stated following semver (Semantic Versioning) 2.0.0 standard.
- In addition, TODO.txt style tag may be used inline in other formats. See
...
- Getting or running 'status' is when we can reduce a rule to a set of codes for specific states, and then summarize on abnormal or expected states. All from cache or loaded data, readily available save for some IPC for retrieval and output format handling. 

  For Bash an invocation can be anything, and all of its exit states are encoded into 0-255. While very convenient in its simplicity, we need to know much more, to know if the invocation *actually* completed and if it encountered any abnormalities.

  If a command takes more than a second or two to complete and return (during which it can inform what is happening) to the user, then it is not useful in interactive scenarios. One second is already very noticeable delay, so for Bash scripts it is especially necessary to prepare and integrate transparent caches when writing routines that collect data, so that those can later be handed over and operated by background or system processes.

  Later, scripts focussed on these user rules can determine *cost* more objectively (by measure and statistic analysis methods) and integrate them with the express purpose of up-front prediction of the cost
  in **time** and **size** for an invocation. And in this case for the scenario where want to summarize and understand status, we can assign costs and priorities to specific data, so that status is finally associated with time windows or periods, "heartbeats" when the actual refresh occurs, two (or nearly two) periods of actual data, and after that only stale data anymore.

  Integrating Redo's incremental builds, and different sorts of suites allows to get a lot a data (specific to the project, build, testing, etc. but also to start thinking about the above; state and schema, and reduction rules).

## Naming conventions

TODO: consolidate into NAME_CONVENTIONS.md:

- Filenames follow standard rules. They can have one or more .ext tag, denoting format, encoding, etc. They should have at least such extension.
...
Refer to [Naming conventions](NAME_CONVENTIONS.md) for a detailed guide on naming.
